...
export const Descartados = () => {
    ...
    const eliminarDeDescartados = async (res) => {
        ...
        try {
            const response = await fetch(`https://scaling-dollop-974v94jqq446fxxw4-3001.app.github.dev/api/descartado/${res.id}`, {
                method: "DELETE",
                headers: {
                    ...
                    {store.descartados.map((res, index) => (
                        <div key={index} className="col-md-4 mb-3">
                            <div className="card p-3 border-danger shadow-sm h-100">
                                ...
                                <button 
                                    className="btn btn-sm btn-link text-danger text-decoration-none p-0 text-start mt-2" 
                                    onClick={() => eliminarDeDescartados(res)}
                                >
