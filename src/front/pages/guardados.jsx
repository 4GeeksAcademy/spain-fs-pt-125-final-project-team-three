...
export const Guardados = () => {
    ...
    const toggleFavorito = async (res, esFav) => {
        ...
        if (esFav) {
            ...
            try {
                const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/favorito/${favToDelete.id}`, {
                    method: "DELETE",
                    headers: { "Authorization": "Bearer " + token }
                });
                if (response.ok) dispatch({ type: "borrar_favorito", payload: res.nombre });
            ...
            try {
                const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/favorito`, {
                    ...
                    body: JSON.stringify({ nombre: res.nombre, tipo: res.tipo || "Restaurante" })
                });
                if (response.ok) {
    ...
    const pasarAVisitado = async (res) => {
        if (!token) return;
        try {
            const responsePost = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/visitado`, {
                method: "POST",
                headers: { "Content-Type": "application/json", "Authorization": "Bearer " + token },
                body: JSON.stringify({ nombre: res.nombre, tipo: res.tipo || "Restaurante" })
            });
            ...
            if (responsePost.ok) {
                ...
                await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/guardado/${res.id}`, {
                    method: "DELETE",
                    headers: { "Authorization": "Bearer " + token }
                });
    ...
    const eliminarDeGuardados = async (res) => {
        if (!token) return;
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/guardado/${res.id}`, {
                method: "DELETE",
                headers: { "Authorization": "Bearer " + token }
            });
            if (response.ok) dispatch({ type: "borrar_guardado", payload: res.nombre });
        } catch (error) { console.error(error); }
    };
                            ...
                            <div key={index} className="col-md-4 mb-3">
                                <div className="card p-3 shadow-sm border-0 h-100">
                                    <div className="d-flex justify-content-between align-items-center">
                                        ...
                                        <button className="btn btn-sm p-0 fs-4" onClick={() => toggleFavorito(res, esFav)}>
                                            {esFav ? "⭐" : "☆"}
                                        </button>
                                    </div>
                                    <p className="text-muted small mt-1 mb-0">{res.tipo}</p>
                                    <div className="mt-auto pt-3">
                                        <button className="btn btn-success btn-sm w-100 mb-2" onClick={() => pasarAVisitado(res)}>✅ Marcar Visitado</button>
                                        <button className="btn btn-outline-danger btn-sm w-100" onClick={() => eliminarDeGuardados(res)}>Eliminar</button>
                                    </div>
                                </div>
                            </div>
                        );
